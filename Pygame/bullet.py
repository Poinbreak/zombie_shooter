...
class bullets(pygame.sprite.Sprite):
    def __init__(bullet,x,y,vel,mosx,mosy):
        bullet.x=x
        bullet.y=y
        bullet.vel=vel
        bullet.targetx=mosx
        bullet.targety=mosy
        bullet.slope=mosx/mosy
        

    def draw(bullet,win,mosx,mosy):
        screen.blit(bullet_image,(bullet.x,bullet.y))
    def update(bullet):
        bullet.x+=bullet.vel
        bullet.y+=bullet.slope*bullet.x
...
while running==True:
    ...
    for event in pygame.event.get():
        if event.type== pygame.QUIT:
            ...
            bullet_group.add(bullets(plyaerx-10,playery-50,3,mousx,mousey))
