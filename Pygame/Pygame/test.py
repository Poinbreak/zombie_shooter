...
class bullets(pygame.sprite.Sprite):
    def __init__(bullet,mosx,mosy):
        ...
        bullet.slope=-(400.1-mosy)/(mosx-400.1)
        bullet.target_angle=math.atan(bullet.slope)

    def update(bullet):
        global enemies
        global no_enem
        if mousx>400:
            bullet.rect.x+=5*math.cos(bullet.target_angle)
            bullet.rect.y+=5*math.sin(bullet.target_angle)
        else:
            bullet.rect.x-=5*math.cos(bullet.target_angle)
            bullet.rect.y-=5*math.sin(bullet.target_angle)
